...
> were approved on 2026-09-24. **Next: Task 1.**
>
...
1. **Read this file top to bottom.** Every decision is here; the conversation that produced it
   is gone.
...
- **This machine (2026-09-23):** Python 3.14.6 with numpy. **Not installed:** scipy, soundfile,
  ffmpeg, sox, oggenc.
...
- [ ] Install: `python -m pip install --user soundfile`. It bundles libsndfile with Vorbis support.
- [ ] Check: `python -c "import soundfile as sf; print(sf.__version__, sf.available_formats().get('OGG'))"`.
- [ ] Round trip, in the scratchpad: write a 1 s sine to OGG/Vorbis, read it back, and compare
      length and RMS.
- [ ] **Fallback** if there's no build for Python 3.14: `winget install ffmpeg` (ask first), and
      encode via `subprocess`.
- [ ] Record the chosen encoder and its version in the **Session log**.
...
- [ ] Write `docs/audio.md` in the style of `docs/design.md` and `docs/ui.md`: the sound list,
...
- [ ] This plan stays the record of intent. `docs/audio.md` becomes the record of what ships,
...
- [ ] Run it: `./gradlew core:test --tests '*<Class>Test'`. Confirm it **fails for the right
      reason**, and use `core:cleanTest` if the task could be skipped as up to date.
- [ ] Implement until green.
- [ ] Commit.

Also:
- [ ] Add `'com.tomer.scoundrel.audio'` to the coverage gate in `core/build.gradle`.
      `CoverageGateTest` fails until this is done.
- [ ] Test cases that must exist:
  - [ ] Weight boundaries for all four scales: 4/5, 7/8 and 5/6, 10/11, and damage 0/1, 4/5.
  - [ ] No version repeats back to back, every version is reachable, and a fixed seed gives a
        fixed sequence.
  - [ ] Every `GameEvent` type has a defined sound choice (none for `WeaponDegraded`,
        `RoomDealt`, `GameWon`, `GameLost`).
  - [ ] A `PotionUsed` with 0 healed gives the drink sound.
  - [ ] A clean weapon kill gives the blade only; a costly one gives the blade plus a thud of
        the right weight.
  - [ ] The expected-file list is exactly the 29 names in the asset contract.
  - [ ] `PendingCues`: fires once on its beat; skip flushes all pending once; pending flips
        collapse into one.
  - [ ] `AudioSettings`: defaults (music 2, sound 3, unmuted), cycling, gains by level (0 = off,
        1 = −12 dB, 2 = −6 dB, 3 = 0 dB), a save/load round trip, and a corrupt or missing file falling back to defaults.
  - [ ] The progress reset leaves the settings file alone (a test in the root package, next to
        `Progress`).
  - [ ] `MusicDirector`:
...
- [ ] **`check.py` additions:** loop-seam continuity (sample jump and spectral click at the
      wrap), loudness relative to the sound effects, duration, and an OGG round trip.
...
- **Python 3.14 is new.** `soundfile` may have no build for it; ffmpeg is the fallback (Task 1).
...
*(none yet: record each step that didn't survive contact with the code, and what was done
instead)*
...
- **2026-09-24:** Plan approved and the nine planning decisions locked. This file written.
  Baseline: 601 `@Test` annotations in 76 classes, purity grep clean, HEAD `51b4da8`.
