...
> in-game sound log. **Next: the user listens for round 3**: in the game, with the F9 lab's
> music keys (R run track, X death, V win), plus the audition page's music section (rebuild
> with `python audio-source/audition.py`). Apply what they say, then Task 7.
>
...
- [ ] **Task 6:** Music, the torch loop and `MusicDirector` → 🎧 **Listening round 3**
      ...
      `4d0a625`, `7a2d9f6`; **waiting on the user's round 3**)
