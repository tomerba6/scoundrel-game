...
> ## ✅ Executed 2026-09-24 — do not re-run
>
> All eight tasks shipped on the **`audio` branch**. It is not merged to `main`, not pushed and not
> released; a release is a separate decision. This file is kept as the record of intent;
> **[`docs/audio.md`](../../audio.md) is the record of what ships.**
>
> **Measured at the close:**
> - 749 `@Test` methods (778 runs) in 90 classes, 0 failures.
> - `audio` at 100% line and 99.4% branch coverage.
> - The purity grep, with `audio` included, is empty.
> - `check.py` passes all 29 effects and 5 streams.
> - All three listening rounds were signed off by the user.
>
> **What did not survive contact with the code, or the ear** (the detail is under **Deviations**):
>
> - **Task 3** grew more classes than planned, with the same responsibilities: `Scale`, `Sound`
>   (which owns the 29-file contract, not `SfxChoice`), `Sfx` and a separate
>   `AudioSettingsStore`. Frequent sounds also vary a little in pitch and level per play.
> - **Task 4:**
>   - Loudness is K-weighted (BS.1770), not plain RMS, which left the thud inaudible.
>   - The versions differ by design, not by chance.
>   - `check.py` does more than planned, and `replaced.txt` protects sourced files.
>   - **Decision 2 became a riffle:** each later card of a deal is quieter and lower.
> - **Task 5:**
>   - The sound log also switches on from the environment, since `gradlew` drops `-D`.
>   - `Sound.voices()` caps the copies of each sound.
>   - `BoardView`'s two-argument `start()` stopped dealing.
>   - A cut-in also plays whatever is pending.
> - **Task 6:**
>   - The contract is named `StreamFile`, since `Stream` clashed with `java.util`.
>   - The torch follows SOUND, not MUSIC.
>   - The F9 lab got music keys: R, X and V.
>   - OGG bytes aren't deterministic, so the checks compare decoded audio.
>   - libsndfile needs writes in blocks.
>   - `beginRunEnd` fixed a silent death after a win.
> - **Listening round 3:**
>   - The torch was rebuilt as crackle over a murmur.
>   - The fist was rebuilt and raised.
>   - The effects couldn't come up (the blade was already at the peak ceiling), so the music came
>     down 6 dB instead.
> - **Task 7:**
>   - `AudioControls` and `LaunchSwitch` are new.
>   - A playing cue follows the level.
>   - The M line under the plates became always visible.
>   - The no-device check covered a run's end with the lab rather than finishing a real run.
>   - **The locked "audio pauses when minimised" was replaced at the user's word.** It went from
>     a hold, to audible playback, to *silent but keeping time*. A zero level is now silence,
>     not a stopped clock.
> - **Task 8** fixed stale figures on the way: the README's test counts and coverage, `docs/ui.md`'s
>   wordmark size and launcher, and the skill's end-panel coordinates.
> - **Not audio, but on this branch:** the end panel's unlocked-trophy description now wraps
>   inside the panel (`069b190`). The user found it.
...
- [x] **Task 8:** Docs sync and memory (2026-09-24: `790cc8a`, `3fb55e6`, `35f5f45`, `f397b99`,
      `becd3a2`, and this file's close)
- [x] ~~**Later, open-ended:** replace weak placeholders from approved sources (see **Sourcing ledger**)~~
      **Closed 2026-09-24 by the user:** "I like the audio we have, so no replacement will be
      made." Every synthesized sound is final.
...
- [x] **`CLAUDE.md`:**
  ...
  - Keep the file under 200 lines. *(198)*
- [x] **`docs/ui.md`:** the title plates, the M key and the audio behaviour.
- [x] **`screens/CLAUDE.md`:** "sounds fire on beats via `Beats`; never on apply".
- [x] **`README.md`** features and a **`CHANGELOG.md`** Unreleased entry.
- [x] **`docs/audio.md`:** switch every *planned* marker to what actually shipped.
- [x] **Memory:** update `audio-direction.md` to *built*.
- [x] **This file:** the banner becomes "Executed", with the deviations.
- [x] *Added:* the `run-scoundrel` skill, since it is the driving reference.
...
- **2026-09-24, Task 7, the user's second look.**
  ...
  - **Task 7 is done.**
- **2026-09-24, not audio: the end panel's trophy text.**
  - **Found:** the user sent a screenshot of Rock Bottom's description running about 65 px past
    the end-of-run panel. It was drawn as one unbounded line (`GameScreen.drawUnlocked`). The
    same bug is on `main`.
  - **Asked:** the user chose to wrap to two lines, on this branch.
  - **Fixed** (`069b190`, tests first): `ScreenArt.END_TEXT_RIGHT` (the rule's right end),
    `endTrophyDescWidth`, and the two lines sharing the seal's height, through `TextWrap`.
  - **Verified:** a rig showed all 10 trophies two at a time; screenshots were taken and the rig
    reverted. Only Rock Bottom wraps, and it stays inside the panel.
  - **Seen, not automation:** a Standard run of the user's own, recorded at 16:57.
- **2026-09-24, Task 8 (docs sync).**
  - **`CLAUDE.md`** (`790cc8a`, 198 lines): `audio` in the package list, the gate, tests-first
    and the purity grep; the regenerate commands and the launch switches; `docs/audio.md` in
    References. Room was made by folding the root package's three classes into one entry and
    tightening the screen-rules pointer.
  - **`screens/CLAUDE.md`:** the sound rule.
  - **`docs/ui.md`** (`3fb55e6`): the volume row, M, the sound summary and the audio files.
    Two stale figures were fixed: the wordmark is 52px (the 26px face at ×2), not 38, and the
    launcher opens borderless-fullscreen.
  - **`README.md` and `CHANGELOG.md`** (`35f5f45`): the feature, a tech row, `audio/` in the
    tree, an audio roadmap block, an Unreleased entry (with the trophy fix under Fixed), and
    every test and coverage figure re-measured. They read 749 methods and 778 runs; `screens`
    at 25.0% line, 28.8% instructions and 33.2% branches; the split at 45 pure and 21 GL-bound
    classes.
  - **`docs/audio.md`** (`f397b99`): nothing left marked planned. The test-class count was
    corrected from eight to eleven.
  - **The `run-scoundrel` skill** (`becd3a2`): the lab's R and V, M, the sound log, the plates'
    coordinates, and the end panel's buttons as one row. The row had been a column, measured
    anew from a screenshot: x 444/573/698/825, at y 533 with trophies, 476 without.
  - **Memory:** `audio-direction.md` now reads *built*, and a new `confirm-ambiguous-requests`.
  - **Final checks:**
    - `core:cleanTest core:check`: 778 runs in 90 classes, 0 failures, `audio` at 100% / 99.4%.
    - The purity grep with `audio` is empty.
    - `check.py` passes everything.
  - **The plan is executed.**
- **2026-09-24, sourcing closed.** "I like the audio we have, so no replacement will be made."
  - The synthesized sounds are final.
  - The open-ended roadmap item is closed.
  - `docs/audio.md`'s Sourcing section is marked closed, and "placeholder" is dropped where it
    described the shipped files.
  - The CHANGELOG, the README roadmap, `CLAUDE.md`, `audio-source/README.md` and the memory note
    say the same.
  - The sourcing rules and `replaced.txt` stay, unused, in case that ever changes.
