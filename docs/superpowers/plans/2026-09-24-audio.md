...
> built and verified in the game** (`6cccf54`…`c0b45d9`). **Next: the user signs off the title
...
- [ ] **Task 7:** Controls: title plates, M, saved settings, pause when minimised, no-device check
      ...
      `c0b45d9`; **waiting on the user's sign-off of the plates' placement**)
...
- [x] **Minimise:** `ScoundrelGame.pause()`/`resume()` pause and resume the streams. Verify from
...
- **The minimise pause is a flag, not `Music.pause()`.** The LibGDX 1.14.2 backend source
...
- **The plates:**
  ...
  - Muted shows "MUTED · M TO UNMUTE" under the row.
