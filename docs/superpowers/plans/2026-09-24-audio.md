...
> ## 🟡 Status: in progress; Tasks 1–6 done, Task 7 built and waiting on the plates' sign-off (2026-09-24)
>
...
> built and verified in the game** (`6cccf54`…`c3f94c9`). **Next: the user signs off the title
> plates' placement from the screenshots**, then Task 8, the docs sync.
>
...
- [ ] **Task 7:** Controls: title plates, M, saved settings, pause when minimised, no-device check
      (built and verified in the game 2026-09-24: `6cccf54`, `71eddaa`, `63a8c84`, `652a013`,
      `c0b45d9`; after the user's first look, the M line `61475a7` and audio playing on while
      minimised `c3f94c9`; **waiting on the user's sign-off of the plates' placement**)
...
- ~~**Audio pauses when the window is minimised.**~~ *Changed by the user on 2026-09-24, after
  seeing it built: the audio plays on with the picture, since a held cue could sound minutes
  after its moment.* With no audio device the game runs silently
  and never crashes.
...
lets the audio play on through a minimise (the user's change, Task 7).
...
- [x] **Minimise:** ~~`ScoundrelGame.pause()`/`resume()` pause and resume the streams.~~ Built
      that way (`652a013`); then the user asked for the audio to play on (`c3f94c9`), and
      `pause()`/`resume()` now only log. Verify from the log inside the running game, not by
      reasoning about the backend.
...
- [ ] **Screenshot the title** and get the user's approval. The mock has no plates, so placement
      is a design sign-off. *(Screenshots taken; waiting on the user.)*
...
- ~~The log shows the music paused and resumed around a minimise.~~ The log shows a cue starting
  and ending while the window is minimised.
...
- **The user reversed the minimise design after seeing it built:** "the pause is less natural,
  ...
  - The audio now plays on with the picture while minimised, even though it's not heard there.
    This replaces the locked design's "audio pauses when the window is minimised".
  - `ScoundrelGame.pause()` / `resume()` only log it, through `MusicDeck.windowMinimised`, so
    the sound log still shows what played while the window was down.
